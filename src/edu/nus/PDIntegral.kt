...
class PDIntegeral(var l1:Double, val l2:Double, val d1:Double, val d2:Double, val I:Int){
    ...
    private fun integeralRect(y:Double, Y:Double, x:Double, X:Double, d:Double, D:Double, Id:Double, ID:Double, sign:Double):Double {
      ...
      val s = (
        + 2*(K*v - L*v - 2*a + 2*b)*v*(log(-K*v - b + p) - log(-K*v - b + q))
        - 2*(K*w - L*w - 2*a + 2*b)*w*(log(-K*w - b + p) - log(-K*w - b + q))
        + 4*(p*v - p*w - q*v + q*w)
        + 2*K*(w - v)*(K*p - L*p - K*q + L*q)
    ...
    private fun integeral11(y:Double, Y:Double, x:Double, X:Double, d:Double, D:Double, sign:Double):Double {
        ...
        val (q,p,w,v)=listOf(x,X,y,Y)
        val (a,b)=listOf(d,D)
        val s= (  v * v / 2 - w * w / 2 + (v - w) * (p - q)
                + (a - b) * (v * log(q - b - v) - w * log(q - b - w))
                - (a - b) * (v - w) * (log(p - b) + 1.0)
    ...
    private fun integeral_11(y:Double, Y:Double, x:Double, X:Double, d:Double, D:Double, sign:Double):Double {
        ...
        val (a,b)=listOf(d,D)
        val s= (
                (p - b) * (v - w)/2 + w*w/2 - v*v/2
                + v*(-a + b + v)*(log(-b + p - 2*v) - log(-b + q - v))
                - w*(-a + b + w)*(log(-b + p - 2*w) - log(-b + q - w))
